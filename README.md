...
Если деплой падает после строки `Found 1 HTTP ports: ['3000/tcp']`, а в логах приложения видно `npm error signal SIGTERM`, укажите в настройках деплоя команду запуска напрямую:

```bash
node server.js
```

а команду сборки оставьте:
